...
const AnnouncerCard = ({ authorName,description,link }: any) => {
    ...
    return (
        <>  
            <Card w={'90%'} maxW='md' css={{boxShadow:'none'}} borderRadius={'4px'} bg={'grey.white'} padding={'15px 0'}>
  <CardBody >
    <Stack css={{display:"flex", alignItems: "center" }}>
        <Avatar color={'grey.white'} size='xl' name={authorName} bgColor={'brand.1'}></Avatar>
    </Stack>
    
    <Stack mt='6' spacing='7'>
      <Heading  size='md' fontSize='h6' textAlign={'center'} noOfLines={1} >{authorName}  </Heading>
